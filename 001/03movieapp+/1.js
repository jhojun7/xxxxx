    ...
    fetch("https://yts.mx/api/v2/list_movies.json?sort_by=rating") //fetch라는 기본함수 만들기 / 비동기로 처리
        ...
        .then((json) => {
            ...
            for (let i = 0; i < movies.length; i++) { // 1개씩 집어넣기
                ...
                //레이팅
                let moiverating = document.createElement("div")
                movies[i].rating >= 9 ?
                    moiverating.setAttribute('class','moiveratinggood')
                    : movies[i].rating >= 7 ?
                    moiverating.setAttribute('class','moiveratingsoso')
                    :moiverating.setAttribute('class','moiveratingbad')
                goodicon = movies[i].rating >= 9 ? "🎈" : ""
                moiverating.innerHTML = `평점 : ${movies[i].rating} / 10점 ${goodicon}`
                parent.appendChild(moiverating)    
